...
class XVLMWrapper:
    def __init__(self, root_dir, device, checkpoint_name, variant="xvlm-coco"):
        # hardcode path for saving in my scratch
        self.scratch_path = "/home/mila/r/rabiul.awal/scratch/compositional_vl"
        self.variant = variant
        self.checkpoint_name = checkpoint_name
        self.root_dir = root_dir
        self.config_path = os.path.join(root_dir, f"{self.variant}-config")
        self.model_path = os.path.join(root_dir, f"{self.checkpoint_name}.pt")
        self.bert_config_path = os.path.join(self.scratch_path, "configs", download_urls[variant]['bert_config_url'].split("/")[-1])
        self.vision_config_path = os.path.join(self.scratch_path, "configs", download_urls[variant]['vision_config_url'].split("/")[-1])
    ...
    def model_name(self):
        return "xvlm_" + self.checkpoint_name
    ...
    def download(self):
        ...
        os.makedirs(os.path.join(self.scratch_path, "configs"), exist_ok=True)
    ...
    @torch.no_grad()
    def get_image_embeddings_custom(self, images):
        image_feats = []
        image_embeds = []
        for image in tqdm(images):
            # convert image to a batch of size 1
    ...
    def predict(self, images: list, texts: list, src_type: str = "local"):
        ...
        images = [self.image_processor(Image.open(image).convert('RGB')) for image in images]

        with torch.no_grad():
            text_embeds, text_ids, text_atts = self.get_text_embeddings(texts)
            image_feats, image_embeds = self.get_image_embeddings_custom(images)
            scores_matrix = image_embeds @ text_embeds.T
